...
import { UserSpotsData } from '../../data/userSpots';
import SingleAdSpot from './singleAdSpot';
import { useSelectedCurrency } from '../../contexts/CurrencyContext';

function MyAdsSpots() {
  const { items, requestSort } = useSortableData(UserSpotsData);
  const { currencyInfo } = useSelectedCurrency();
  return (
    <div className="md:px-16 lg:flex-row px-4 my-10">
          ...
          <thead className=" bg-transparent">
            <tr className="px-4">
              ...
              <th className="whitespace-nowrap px-4 py-2">
                ...
                >
                  <span className="float-left mr-2">Status</span>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="8"
                    height="15"
